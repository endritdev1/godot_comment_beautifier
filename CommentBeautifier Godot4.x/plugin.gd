...
func _ready():
	get_viewport().connect("gui_focus_changed", Callable(self, "_on_gui_focus_changed2"))
...
func _on_gui_focus_changed2(nodo):
	if nodo is TextEdit:
		if is_instance_valid(script_editor):
			script_editor.disconnect("caret_changed", Callable(self, "_on_cursor_changed"))
		script_editor = nodo
		script_editor.connect("caret_changed", Callable(self, "_on_cursor_changed"))
